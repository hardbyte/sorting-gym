...
def test_function_env_preserves_function_id():
    ...
    obs, reward, terminated, truncated, info = env.step((3, 1))
...
def test_function_env_can_pass_through_arg():
    ...
    # returning ID r=2
    obs, reward, terminated, truncated, info = env.step((3, 0, 0, 1, 2))
    assert obs['current_function'] == 0
    assert env.v[1] == 0
    assert env.v[2] == 0
    ...
    obs, reward, terminated, truncated, info = env.step((2, 1, 0))
    assert env.v[1] == n
    assert env.v[2] == 0
    # Now return from the function with local variable (id 1).
    # Returning ID is 2, so now v[2] should be n
    obs, reward, terminated, truncated, info = env.step((4, 1))
    assert env.v[2] == n
...
def test_function_env_swap_args():
    ...
    # Assign v1 = v2
    obs, reward, terminated, truncated, info = env.step((2, 1, 2))

    assert env.v[0] == 2
    assert env.v[1] == 1

    # Now return from the function.
    obs, reward, terminated, truncated, info = env.step((4, 0, 1))
...
def test_function_env_swap_args_in_call():
    ...
    obs, reward, terminated, truncated, info = env.step((4, 1, 0))
